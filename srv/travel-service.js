  ...
  init() {
    ...
    this.on("getBookingDataOfPassenger", async (req) => {
      ...
      const allCustomerBookings =
        await SELECT`BookingStatus_code as status`.from(Booking)
    ...
    this.before("UPDATE", "Travel.drafts", async (req) => {
      if ("BookingFee" in req.data) {
        const { status } = await SELECT.one`TravelStatus_code as status`.from(
          req.subject,
        );
        if (status === "A")
          req.reject(
    ...
    this._update_totals_supplement = async function (booking) {
      const { totals } =
        await SELECT.one`coalesce (sum (Price),0) as totals`.from(
    ...
    this._update_totals4 = function (travel) {
      return UPDATE(Travel.drafts, travel)
        .alias("T")
        .with({
          TotalPrice: CXL`coalesce (T.BookingFee, 0) + ${SELECT`coalesce (sum (B.FlightPrice + ${SELECT`coalesce (sum (BS.Price),0)`
            .from(BookingSupplement.drafts)
            .alias("BS").where`BS.to_Booking_BookingUUID = B.BookingUUID`}),0)`
            .from(Booking.drafts)
            .alias("B").where`B.to_Travel_TravelUUID = T.TravelUUID`}`,
        });
    };
    ...
    this.before("SAVE", "Travel", async (req) => {
      ...
      const res = await SELECT.from(Booking.drafts)
